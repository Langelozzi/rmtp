...
generate_cmake_content() {
    ...
    echo "target_link_options($entity PRIVATE \${INSTRUMENTATION_FLAGS_LIST} -Wl,-no_fixup_chains)" >>"$output_file"
    echo "" >>"$output_file"
...
# Additional code
{
    ...
    echo "target_link_directories($first_target PRIVATE /usr/local/lib\${LIBSUFFIX})" >>"$output_file"
    echo "target_link_options($first_target PRIVATE \${INSTRUMENTATION_FLAGS_LIST})" >>"$output_file"
    echo "" >>"$output_file"
    ...
    echo "add_custom_command(" >>"$output_file"
    echo "    TARGET $first_target POST_BUILD" >>"$output_file"
    echo "    COMMAND \${CPPCHECK} --force --quiet --inline-suppr --library=posix --enable=warning,style,performance,portability --suppress=missingIncludeSystem --suppress=unusedFunction --suppress=unmatchedSuppression --suppress=checkersReport -I\${CMAKE_SOURCE_DIR}/include -I/usr/local/include \${SOURCES} \${HEADERS}" >>"$output_file"
    echo "    WORKING_DIRECTORY \${CMAKE_SOURCE_DIR}" >>"$output_file"
