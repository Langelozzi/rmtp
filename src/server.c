...
static void run_server(const server_args_t *args) {
  ...
  // Socket data
  int socket_fd;
  struct sockaddr_storage listen_addr_info;
  struct sockaddr *listen_addr, client_addr;
  ...
  while (running) {
    ...
    if (expected_seq_num < 0) {
      ...
      send_ack(socket_fd, &recvd_packet, &client_addr, client_addr_len);
      continue;
    } else if (recvd_seq_num != expected_seq_num) {
    ...
    if (handle_correct_packet_received(socket_fd, &recvd_packet, &client_addr,
                                       client_addr_len) < 0) {
